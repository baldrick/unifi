...
def sync_unifi_talk(ctx, additive, concatenate, contacts):
    api = TalkAPI(ctx)
    ...
    if not additive:
        ...
        if not response:
            ...
            return
        if ids is not None:
            logger.info(f'deleted all {len(ids)} contacts from Unifi Talk')
        else:
    ...
    if not additive:
        ...
        if len(failed) > 0:
            logger.error(f'deleted {len(deleted)} but failed to delete {len(failed)} unused contact lists from Unifi Talk')
            return
        if len(deleted) > 0:
            logger.info(f'deleted {len(deleted)} unused contact lists from Unifi Talk')
        else:
            logger.info('no unused contact lists to delete from Unifi Talk')
